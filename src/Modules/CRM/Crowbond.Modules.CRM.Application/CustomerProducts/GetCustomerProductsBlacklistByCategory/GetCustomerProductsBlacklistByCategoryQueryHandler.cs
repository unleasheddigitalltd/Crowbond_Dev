...
internal sealed class GetCustomerProductsBlacklistByCategoryQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<GetCustomerProductsBlacklistByCategoryQuery, CustomerProductsResponse>
{
    public async Task<Result<CustomerProductsResponse>> Handle(GetCustomerProductsBlacklistByCategoryQuery request, CancellationToken cancellationToken)
    {
        ...
        string sql =
            ...
            WITH FilteredCustomerProducts AS (
                ...
                WHERE cp.customer_id = @CustomerId AND c.id = @CategoryId AND is_deleted = false
