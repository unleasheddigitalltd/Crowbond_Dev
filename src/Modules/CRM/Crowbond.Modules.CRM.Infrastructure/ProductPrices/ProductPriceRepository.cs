...
public sealed class ProductPriceRepository(CrmDbContext context) : IProductPriceRepository
{
    ...
    public async Task<IEnumerable<ProductPrice>> GetForProduct(Product product, CancellationToken cancellationToken = default)
    {
        return await context.ProductPrices.Where(p => p.ProductId == product.Id).ToListAsync(cancellationToken);
    }
