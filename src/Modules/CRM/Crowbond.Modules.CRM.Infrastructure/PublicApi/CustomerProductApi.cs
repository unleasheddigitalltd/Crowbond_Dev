...
using Crowbond.Modules.CRM.Application.CustomerProducts.GetCustomerProductBySku;
...
internal sealed class CustomerProductApi(ISender sender) : ICustomerProductApi
{
    ...
    public async Task<CustomerProductResponse> GetBySkuAsync(Guid customerId, string productSku, CancellationToken cancellationToken = default)
    {
        Result<CustomerProductPriceResponse> result = await sender.Send(new GetCustomerProductBySkuQuery(customerId, productSku), cancellationToken);
