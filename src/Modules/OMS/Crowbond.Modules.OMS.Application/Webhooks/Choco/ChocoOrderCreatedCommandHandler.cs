    ...
    : ICommandHandler<ChocoOrderCreatedCommand>
{
    public async Task<Result> Handle(ChocoOrderCreatedCommand request, CancellationToken cancellationToken)
    {
        ...
        string postcode = !string.IsNullOrEmpty(orderData.Customer.DeliveryAddress.PostalCode)
            ...
            : Regex.Match(orderData.Customer.DeliveryAddress.Full,
                @"[A-Z]{1,2}[0-9R][0-9A-Z]? [0-9][ABD-HJLNP-UW-Z]{2}").Value;

        var outlet = await customerApi.GetOutletForOrderByPostcodeAsync(postcode, customer.Id, cancellationToken);
        if (outlet is null)
        {
            logger.LogWarning("Outlet not found for customer {CustomerId} at postcode {Postcode}.", customer.Id,
                postcode);
        ...
        {
            logger.LogWarning(
                "Invalid outlet for customer: OutletCustomerId={OutletCustomerId}, CustomerId={CustomerId}.",
                outlet.CustomerId, customer.Id);
        ...
        foreach (var product in products)
        {
            try
            {
                ...
                if (custProd is null)
                {
                    ...
                    logger.LogWarning("Customer product not found: {Sku} for customer {CustomerId}.",
                        product.Product.ExternalId, customerId);
                    errors.Add(err);
                    continue;
                }
                ...
                {
                    logger.LogWarning("Invalid tax rate type: {TaxRateType} for product {ProductId}.",
                        custProd.TaxRateType, custProd.ProductId);
                ...
                if (order.Lines.Any(l => l.ProductId == custProd.ProductId))
                {
                    ...
                    logger.LogWarning("Duplicate product line: {ProductId} in order {OrderId}.", custProd.ProductId,
                        order.Id);
                    errors.Add(err);
                    continue;
                }
            ...
            {
                logger.LogError(ex, "Exception adding product {Sku} to order {OrderId}.", product.Product.ExternalId,
                    order.Id);
