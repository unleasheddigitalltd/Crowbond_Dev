...
internal sealed class UpdateStockLocation : IEndpoint
{
    ...
    {
        app.MapPut("stocks/{id}/relocate", async (ClaimsPrincipal claims, Guid id, Request request, ISender sender) =>
        {
            Result<Guid> result = await sender.Send(new UpdateStockLocationCommand(claims.GetUserId(), id, request.TransactionNote, request.Reason, request.Quantity, request.Destination));
