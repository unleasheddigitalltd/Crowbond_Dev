...
internal sealed class StartPutAwayTask : IEndpoint
{
    ...
    {
        app.MapPut("task/putaway/{id}/start", async (ClaimsPrincipal claims, Guid id, ISender sender) =>
        {
            Result result = await sender.Send(new StartPutAwayTaskCommand(claims.GetUserId(), id));
